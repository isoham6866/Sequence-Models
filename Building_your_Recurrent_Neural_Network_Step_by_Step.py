"""
Created on Wed September 17 15:22:29 2018
@author: soham ghosh
"""
...
def lstm_cell_backward(da_next, dc_next, cache):
    ...
    dcct = (dc_next*it)+(da_next*ot*(1-np.tanh(c_next)**2)*it*cct*(1-cct**2))
    ...
    da_prev = np.dot(Wf[:,:n_a].T,dft)+np.dot(Wo[:,:n_a].T,dot)+np.dot(Wc[:,:n_a].T,dcct)+np.dot(Wi[:,:n_a].T,dit)
    dc_prev = (dc_next*ft)+(da_next*ot*(1-np.tanh(c_next)**2)*ft)
    dxt = np.dot(Wf[:,n_a:].T,dft)+np.dot(Wo[:,n_a:].T,dot)+np.dot(Wi[:,n_a:].T,dit)+np.dot(Wc[:,n_a:].T,dcct)
    ### END CODE HERE ###
...
def lstm_backward(da, dc, caches):
    ...
    for t in reversed(range(T_x)):
        ...
        gradients = lstm_cell_backward(da[:,:,t],dc[:,:,t],caches[t])
...
da = np.random.randn(5, 10, 4)
dc = np.random.randn(5, 10, 4)
gradients = lstm_backward(da, dc, caches)
